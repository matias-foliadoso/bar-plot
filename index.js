...
// Horizontal domain.
const horizontalDomain = data.map(d => d[0])

// Vertical minimum and maximum domain.
const minVerticalDomain = 0
let maxVerticalDomain = data.reduce((max, d) => {
    if (max < d[1]) {
        return d[1]
    }
    return max
...
const horizontalAxis = d3
    ...
    .tickValues(data.map(d => d[0]).filter((d, i) => d.split('-')[0] % 5 == 0 && i % 4 == 0))
    .tickFormat(d => d.split('-')[0])

// Vertical scale and axis.
const verticalScale = d3
    .scaleLinear()
    .domain([minVerticalDomain, maxVerticalDomain])
    .range([height, 3])
...
// Bars appending
const bandWidth = horizontalScale.bandwidth()
const bars = svg
    .selectAll('rect')
    .data(data)
    .enter()
    ...
    .attr('y', d => verticalScale(d[1]))
    .attr('height', d => height - verticalScale(d[1]))
...
bars
    .on('mouseover', e => {
        ...
        const {gdp, date, index} = e.target.dataset
        ...
        tooltip
            .attr('data-date', date)
            .style('left', `${bandWidth * index + 0}px`)
